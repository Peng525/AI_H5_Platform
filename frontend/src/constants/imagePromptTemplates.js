...
export const IMAGE_PROMPT_FIELD_LABELS = ['主体', '环境', '光线', '风格']

/**
 * @typedef {{ label: string, value: string }} ImagePromptField
 * @typedef {{
 *   id: string
 *   title: string
 *   description?: string
...
/** 将模板格式化为填入对话框的文本 */
export function formatImagePromptTemplate(template) {
  if (!template?.fields?.length) return ''
  return template.fields.map((f) => `${f.label}：${f.value}`).join('\n')
}

export function getImagePromptTemplate(id) {
  return IMAGE_PROMPT_TEMPLATES.find((t) => t.id === id)
}
