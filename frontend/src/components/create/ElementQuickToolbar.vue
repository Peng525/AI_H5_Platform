<template>
  <div
    ...
    data-editor-chrome
    class="flex items-center gap-1 bg-white shadow-lg rounded-xl px-2 py-1.5 border border-outline-variant pointer-events-auto"
    :style="barStyle"
    @mousedown.stop
  >
    ...
    <template v-else-if="selected.type === 'chartStack'">
      <button
        type="button"
        class="px-2 py-1 text-xs font-medium rounded border border-outline-variant hover:bg-surface-container"
    ...
    <button type="button" class="p-1.5 rounded hover:bg-surface-container text-on-surface-variant" title="复制" @click="$emit('duplicate')">
      <span class="material-symbols-outlined text-[18px]">content_copy</span>
    </button>
    <button type="button" class="p-1.5 rounded hover:bg-surface-container text-on-surface-variant" title="置顶" @click="$emit('bring-front')">
      <span class="material-symbols-outlined text-[18px]">vertical_align_top</span>
    </button>
    <button type="button" class="p-1.5 rounded hover:bg-surface-container text-on-surface-variant" title="置底" @click="$emit('send-back')">
      <span class="material-symbols-outlined text-[18px]">vertical_align_bottom</span>
    </button>
    <button type="button" class="p-1.5 rounded hover:bg-red-50 text-red-600" title="删除" @click="$emit('delete-selected')">
...
const barStyle = computed(() => floatingBarStyle(props.anchorRect, 'below', 8))
