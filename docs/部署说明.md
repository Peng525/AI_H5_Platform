...
3. **管理**：`/admin` 查看访问与订单趋势；待支付订单区确认收款；或在 `/admin/users` 直接改会员
4. **中转充值**：`/admin` →「前往中转平台钱包」（需配置 `RELAY_DASHBOARD_RECHARGE_URL`）；余额在中转站控制台登录查看
5. **H5 模板**：`/admin/templates` — 编辑或上传 .pptx 解析上架
6. **提示词**：`/admin/prompts`、`/admin/image-prompts`
