...
- 环境变量：`RESUME_MAX_PER_USER`、`RESUME_MAX_FILE_MB`、`RESUME_PADDLEOCR_ENABLED` 等见 `.env.example`
- 默认 `RESUME_PADDLEOCR_ENABLED=false`：PDF 走 pymupdf 文本层；图片 OCR 需启用 Paddle 并增大镜像
