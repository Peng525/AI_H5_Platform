...
**对话生成器 / 文字云等新前端功能**：无需在本机安装 Node.js；改代码后重新构建镜像即可（见下文「日常更新」）。
...
1. **登录**：`CAPTCHA_PROVIDER=mock` → 开发模式验证 → `demo@ai-h5.com` / `demo123456`
2. **支付**：`/upgrade` → 微信扫码 → 展示收款码 → `/admin` 确认收款
3. **管理**：确认收款后开通 Pro；或在 `/admin/users` 直接改会员
4. **H5 模板**：`/admin/templates` — 编辑或上传 .pptx 解析上架
5. **提示词**：`/admin/prompts`
...
1. **AI 助手** → 画面描述 → **AI 生图**（先生成原图，不必提前选尺寸）
2. **原图预览** 下方选择：**适应宽度** / **填充页面** / **原始尺寸** → 添加到页面
3. 已添加图片：选中后画布工具栏 **适应宽 / 填充 / 原图** 可切换

配置 `LLM_RELAY_*` 或 `LLM_OFFICIAL_*` 及 `LLM_IMAGE_MODEL_*`。
...
- **层级**：工具栏置顶 / 置底 / 上移 / 下移
- **管理端版式**：`/admin/layouts` 自定义 JSON 版式
