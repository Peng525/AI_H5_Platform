...
| Deck 胶囊 | 原生 `<select>` ×4 | 页数/背景/尺寸/语言 → 草稿字段 |
| Image 比例 | `ImageAspectRatioSelect.vue` | 6 种比例（图标+数字）；`v-model` → `imageAspectRatio` |
| Image 颜色/风格 | 原生 `<select>` ×2 | `imageColor`、`imageStyle`（风格默认「无」） |
| 胶囊容器 | `.generate-pills-bar` | `sticky top-0`；deck/image **位置一致**，滚动时贴顶 |
| 提示词输入 | `textarea` + `resizeTopicInput` | 单行起，按 `scrollHeight` 增高；粘贴后滚顶 |
| 示例区 | `EXAMPLE_PROMPT_GROUPS` | 仅 `deck && !hasTopic`；「换一组」轮换 |
| 模板区 | `useImagePromptTemplates` | 仅 `image && !hasTopic`；3 列卡片 |
...
| 状态 | 条件 | 可见 UI |
|------|------|---------|
| **空态** | `!topic.trim()` | 胶囊（sticky 顶）→ 输入框 → 分割线 → deck 示例 **或** image 三模板 |
...
| Tab | `type` | 胶囊 | 空态扩展区 | `goNext` 目标 |
|-----|--------|------|------------|---------------|
| 演示文稿 | `deck` | 4 个 select | 6 条示例（2×3 网格） | `/create/generate/review` |
| 生成图片 | `image` | 比例+颜色+风格 | 3 张模板卡片 | `/create/generate/image` |
...
| 控件 | 绑定 | 草稿字段 | 下游 |
|------|------|----------|------|
| 页数 | `pageCount` | `pageCount` | review → 全量生成 `page_count` |
| 背景 | `background` | `background` | `background_preset` |
| 尺寸 | `viewportMode` | `viewportMode` | `auto` / `web` / `mobile` |
| 语言 | `language` | `language` | LLM `language` |

样式：`.pill-select` + `.pill-chevron`（Material `expand_more`）。
...
### 7.2 比例下拉 — `ImageAspectRatioSelect.vue`

- 触发器：圆角胶囊 + 矩形图标 + 比例文字 + chevron。
- 浮层：check + 图标 + 比例；点击外部关闭。
- `v-model`：比例字符串（如 `9:16`）。
- 用于：生成页 image Tab、[`AiGenerateImage.vue`](../frontend/src/views/create/AiGenerateImage.vue) 左栏「尺寸」。

### 7.3 颜色 / 风格

- 原生 select，与 deck 胶囊视觉一致。
- 选模板时：若 `tpl.suggestedStyle` 合法 → 同步 `imageStyle`。
- 风格为「无」时：`enrichImagePrompt` **不**追加风格行。
...
- 数据源：`EXAMPLE_PROMPT_GROUPS`（2 组 × 6 条，循环）。
- 点击卡片：`applyExample` → 写入 `topic`，清空模板选中态。
- 「换一组」：`exampleGroup++`。

### 8.2 生图模板

- 数据源：[`useImagePromptTemplates`](../frontend/src/composables/useImagePromptTemplates.js)（API 或 `IMAGE_PROMPT_TEMPLATES` fallback）。
- 点击：`formatImagePromptTemplate(tpl)` 写入 `topic`；选中 `check_circle`。
- 布局：`md:grid-cols-3`，与输入框同宽；小屏横向滑动。
...
sequenceDiagram
  ...
  alt type image
    ...
    Start->>Draft: saveDraft pageCount background viewportMode language topic
...
| `imageColor` | `classic_white` | 拼入 prompt 色调 |
...
| `frontend/src/components/create/ImageAspectRatioSelect.vue` | 比例胶囊下拉 |
...
| 2026-05-31 | 胶囊 sticky 固定于输入框上方；输入框纯内容增高；移除 fillHeight/底部胶囊 |
