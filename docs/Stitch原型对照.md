# Stitch 原型与实现对照

原型项目：https://stitch.withgoogle.com/projects/11380331898068582338

详细需求见 [需求清单.md](./需求清单.md)。

## 用户端页面

| Stitch 页面 | 路由 | 状态 |
|-------------|------|------|
| 登录/注册 + 拼图验证 | `/login` | ✅ 滑块拼图 |
| 探索模板 + 终端/类型筛选 | `/templates` | ✅ 工作台侧栏「模板库」 |
| 我的工作台 | `/dashboard` | ✅ `WorkbenchShell` 双栏侧栏；首页项目列表 |
| 工作台导入 PPT | 顶栏「导入 PPT」 | ✅ `POST /api/v1/项目/导入-pptx` |
| 三栏编辑器 + 可视化拖拽 | `/editor/:id` | ✅ localStorage 持久化 |
| 编辑器素材（PPT 组件） | 工具箱 · 素材 | ✅ 文本框/矩形/表格/图标/图片/图表 |
| 页面画布背景色 | 素材面板 | ✅ 按页保存 |
| Word 风格文本工具栏 | 画布顶部工具栏 | ✅ 字体/字号/颜色/行距等 |
| 版式叠加与组件层级 | 素材 · 版式 + 画布工具栏 | ✅ 版式追加不替换；置顶/置底/上移/下移 |
| 对话播放方式 | 对话生成器 | ✅ 点击播放 / 自动播放二选一 |
| 对话生成器三栏布局 | 全屏 Modal | ✅ 窄屏保持三栏；手机预览等比缩放 |
| AI 面板（通道/配额/生图分辨率/适应方式/裁切） | 编辑器右侧 `AiPanel` | ✅ 生图前选分辨率；原图预览后自选适应宽/填充/原图；工具栏可改与裁切 |
| AI 创建向导（三步） | `/create/generate` → `/prompt` → `/review` | ✅ 登录默认页；`POST /项目/ai-生成` |
| 套餐升级 + 扫码支付 | `/upgrade` | ✅ 本地 demo 支付闭环 |
| 发布成功 + 链接/二维码 | `/publish/:id` | ✅ 真实 QR |
| H5 全屏预览（手机框切换） | `/preview/:id` | ✅ |
| 分享页 | `/s/:slug` | ✅ |
| 全局导航 + 登出 | `WorkbenchShell` / `AppShell` | ✅ 工作台 + 编辑器顶栏 |

## 管理员端（不对普通用户展示）

| 功能 | 路由 | 状态 |
|------|------|------|
| 管理控制台框架 | `AdminShell` | ✅ 侧栏固定高度 + 底部返回/退出常驻 |
| 数据仪表盘（访问/订单/成交额/趋势图/待确认订单） | `/admin` | ✅ |
| 中转 API 充值外链 | `/admin` | ✅ 不自动读余额，跳转钱包页 |
| 用户管理（创建/编辑/配额） | `/admin/users` | ✅ |
| H5 模板管理 | `/admin/templates` | ✅ 列表 + 可视化编辑（EditorStudio 复用） |
| 模板保存预设 / PPT 导入 | 编辑器顶栏（adminPreset 模式） | ✅ |
| 版式管理 | `/admin/layouts` | ✅ 可视化编辑（EditorStudio）；14 内置 + 自定义 |
| 文稿提示词模板编辑 | `/admin/prompts` | ✅ |
| 生图提示词模板 | `/admin/image-prompts` | ✅ |
| 大模型配置（在线保存 `.env`） | `/settings` | ✅ 仅管理员 |
| 管理员登录跳转 | `/login` → `/admin` | ✅ |

## 设计规范

- 主色 `#005daa`、背景 `#fcf9f8`（见 `_proto_web/.../DESIGN.md`）
- 字体 Inter + Material Symbols
- 免费档：Gemini 3.1 Flash；升级档：Gemini 3 Pro

## 延后（上云 / 公网回调）

- 微信/支付宝真实支付回调
- 微信 OAuth 登录
