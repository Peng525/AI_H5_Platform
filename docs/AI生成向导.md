...
| 步骤 | 路由 | 说明 |
|------|------|------|
| 1 | `/create/generate` | **Gamma 单页两态**：空态单行输入 + 示例（deck）或 **3 张生图模板卡片**（image）；有输入后隐藏示例/模板，居中 **「编辑提示词」** |
...
| 状态 | 演示文稿 | 生成图片 |
|------|----------|----------|
| 空态 | 胶囊配置 + 单行输入 + 示例卡片（可换一组） | 单行输入 + **3 张生图模板**（主体/环境/光线/风格全文） |
| 有输入 | 隐藏示例，居中「编辑提示词」 | 隐藏模板，居中「编辑提示词」 |

- 输入框随内容自动增高（最高约 192px，超出后框内滚动）
