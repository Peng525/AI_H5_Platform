...
| 步骤 | 路由 | 说明 |
|------|------|------|
| 1 | `/create/generate` | **单页**：类型 Tab（默认演示文稿）、胶囊配置（左对齐，仅 deck）、大输入框（deck/image 共用）、示例提示（仅 deck）、「继续」 |
| 2 | `/create/generate/review` | 提示编辑器：文本量、内容模式、受众、语气、扩写与附加说明 |
| 完成 | `/editor/:id` | 生成成功后进入三栏编辑器 |

**图片类型** 在生成页选「生成图片」→ 填写提示词 →「继续」进入 `/create/generate/image`；调用 `POST /api/v1/生图/独立`，**不创建项目、不进编辑器**。

> `/create/generate/prompt` 已合并至 `/create/generate`，旧链接自动重定向。

## 图片生成页（`/create/generate/image`）

桌面端 **左右等宽两栏**（`lg:grid-cols-2`），窄屏上下堆叠。

| 左栏 · 提示词编辑 | 右栏 · 生成结果 |
|-------------------|-----------------|
| 提示词 textarea（预填生成页 `topic`） | 展示本次生成所用提示词 |
| 尺寸：移动端 / 传统网页 | 图片预览框（空态 / 加载 spinner / 结果） |
| **生成图片**（旋转加载） | **点击图片** → 裁切弹窗 |
| | **复制** → 剪贴板 |
...
1. 登录后进入 `/create/generate`：渐变背景、Tab（默认演示文稿）、左对齐胶囊、大输入框；顶栏用户名下方见配额
2. 演示文稿：填提示词 → 继续 → 提示编辑器 → 生成 → 编辑器页数与背景正确
3. 生成图片：选类型 → 填提示 → 继续 → 两栏图片页 → 生成 → 右栏显示结果；点击裁切、复制可用
