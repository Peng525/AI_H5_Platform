...
> 需求追踪见 [`需求清单.md`](./需求清单.md) 第九节（DG-01~08、WC-01~06）。  
> 图表卡组见 [`互动组件-图表卡组.md`](./互动组件-图表卡组.md)（CS-01~06）。
...
| 数据存储 | `slides.chat_script_json` | `enabled`、`playbackMode`、`autoAdvanceMs`、`timeline[]` |
...
| 字段 | 说明 |
|------|------|
| `playbackMode` | `click` 点击逐句；`auto` 自动逐句（配合 `autoAdvanceMs`，默认 1500ms，范围 100～60000） |
| `autoAdvanceMs` | 自动模式下间隔毫秒数；点击模式下为 `0`（兼容旧数据：仅看此字段亦可推断模式） |
...
- **点击播放**（`playbackMode: click`）：用户点击屏幕逐句显示；完成后允许翻页
- **自动播放**（`playbackMode: auto`）：进入对话页后按 `autoAdvanceMs` 自动推进；仍可点击跳过加速
- `timestamp` 随上一条消息一起显示，不参与单独 reveal 步进
- 对话生成器右栏提供 **点击播放 / 自动播放** 二选一；选自动时可配置间隔（ms，失焦后校验，支持清空后输入如 `200`）

### 2.6 全屏编辑器布局

- **三栏固定**：左样式预设、中手机预览、右圆角/颜色/参与者/播放方式，缩小窗口时**不改为纵向堆叠**
- **手机预览**：`PhoneDeviceFrame` 使用 `fit=contain`，在可用高度内**等比缩放**竖屏画框
- **极窄屏**：三栏整体可横向滚动（`min-width` 约 44rem），避免预览区被压扁
...
**缺口（已修复）**：默认网页视口（如 `web-wide-1024`）下，对话生成器预览固定手机框；插入后结果页卡片因 `PreviewSlideFrame` 未渲染 `chat_script` 而显示空白。

**目标 UX**：

- 生成器中央预览随项目 `viewportId` 切换手机框 / 浏览器框（复用 `PhoneDeviceFrame` 的 `device` / 宽高）。
- 结果页缩略图、卡片画布、编辑态均叠加 `DialoguePreviewCanvas`（与 studio 模式一致）。
- 分享/预览 `ChatStoryOverlay` 播放逻辑不变。
