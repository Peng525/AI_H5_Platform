...
|-----|--------|----------|------------|---------------|
| 演示文稿 | `deck` | 比例 → 色调 → 页数 → 语言 | ppt-master PPT 模板（`DeckPptTemplatePicker`） | `/create/generate/review` → premium 任务 |
...
- 数据源：[`DeckPptTemplatePicker.vue`](../frontend/src/components/create/DeckPptTemplatePicker.vue) → `GET /api/v1/演示/ppt-模板`（内置 layouts + decks + 用户 `data/imported_deck_templates`）。
- **分类 Chip**（默认「品牌模板」）：品牌模板 / 结构模板 / 我的导入 / 全部；复用 `ResumeIndustryChips.vue`，避免一次性展示全部模板。
- **预览图**：`GET /api/v1/演示/ppt-模板/preview/{kind}/{slug}` 返回封面 `01_cover.svg`（浏览器直接渲染，不再依赖 PNG 占位图）。
- 输入框左上角 **「导入 PPT 模板」**：`POST /api/v1/演示/ppt-模板/导入`。
- 交互：与简历视觉模板一致（卡片 → 弹窗预览 → 确认选择）；选中 id 写入草稿 `pptTemplateId`。
- 生成：review 页提交时带 `ppt_template_id`，走 `POST /api/v1/项目/ai-生成-premium`（Worker 全自动）。
- **Premium 进度/ETA**：进度条与「第 k/n 页」对齐；等待 overlay 先显示区间 **「6–10 分钟」**（5 页），进入逐页生成后显示阶段加权的「剩余约」并经 `smoothEtaSeconds` 平滑（见 [`deckGenerateEstimate.js`](../../frontend/src/utils/deckGenerateEstimate.js)）。
- 批量入库：`scripts/import_pptx_deck_templates.py`。

### 8.2.1 主页 Tab 默认

- 进入 `/create/generate` 时默认选中 **演示文稿** Tab；仅 URL 带 `?tab=resume-edit|resume-optimize` 时进入简历 Tab，不再从 `resumeDraft.tab` 恢复。
