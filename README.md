...
- **三栏编辑器**：左侧工具箱（页面 / 文本 / 动效 / 素材，2×2 Tab）+ 中间画布 + 右侧 AI 面板
- AI 全量生成演示结构（模板：全量生成）
- AI 单页改写（模板：单页改写）
...
| 组件 | 要点 |
|------|------|
| **对话生成器** | 响应式三栏；微信椭圆气泡；+ 菜单「添加时间戳 / 添加用户」；侧栏编辑内容与气泡实时同步；头像上传/默认 |
...
- 素材面板 → **商务版式 / 叙事版式** 一键套用
- 新建页面 → 选择版式（可跳过为空白页）
- 动效面板 → 浏览模式、页动效、**BGM**（文本按钮启用 → 选曲；画布右上角旋转播放器）

**工具箱 Tab**：默认「页面」；已移除「模板」入口（浏览模板请用顶栏「探索模板」）。
...
| 编辑器配置 | 工具箱 → **动效** → 点击「启用背景音乐」→ 选择曲目 |
| 播放控制 | 画布右上角 **旋转唱片按钮**：播放时旋转，点击静音/继续；预览与分享页同步 |
